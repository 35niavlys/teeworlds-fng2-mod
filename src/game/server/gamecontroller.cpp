...
bool IGameController::OnEntity(int Index, vec2 Pos)
{
	...
	else if(!m_Config.m_SvSuppressEntities)
	{
...
void IGameController::CycleMap()
{
	if(m_aMapWish[0] != 0)
	{
		...
		GameServer()->Console()->Print(IConsole::OUTPUT_LEVEL_DEBUG, "game", aBuf);
		str_copy(m_Config.m_SvMap, m_aMapWish, sizeof(m_Config.m_SvMap));
		m_aMapWish[0] = 0;
		m_RoundCount = 0;
		return;
	}
	if(!str_length(m_Config.m_SvMaprotation))
		return;

	if(m_RoundCount < m_Config.m_SvRoundsPerMap-1)
	{
		if(m_Config.m_SvRoundSwap)
			GameServer()->SwapTeams();
		return;
	}

	// handle maprotation
	const char *pMapRotation = m_Config.m_SvMaprotation;
	const char *pCurrentMap = m_Config.m_SvMap;
	...
	GameServer()->Console()->Print(IConsole::OUTPUT_LEVEL_DEBUG, "game", aBuf);
	str_copy(m_Config.m_SvMap, &aBuf[i], sizeof(m_Config.m_SvMap));
}
...
void IGameController::TogglePause()
{
	...
	{
		// unpause
		if(m_Config.m_SvUnpauseTimer > 0)
			m_UnpauseTimer = m_Config.m_SvUnpauseTimer*Server()->TickSpeed();
		else
		{
			GameServer()->m_World.m_Paused = false;
			m_UnpauseTimer = 0;
...
void IGameController::Tick()
{
	...
	// do team-balancing
	if(IsTeamplay() && m_UnbalancedTick != -1 && Server()->Tick() > m_UnbalancedTick+m_Config.m_SvTeambalanceTime*Server()->TickSpeed()*60)
	{
	...
	if(m_Config.m_SvInactiveKickTime > 0)
	{
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
		#ifdef CONF_DEBUG
			...
			{
				if(i >= MAX_CLIENTS-m_Config.m_DbgDummies)
					break;
			}
		#endif
...
bool IGameController::CanJoinTeam(int Team, int NotThisID)
{
	...
	return (aNumplayers[0] + aNumplayers[1]) < Server()->MaxClients()-m_Config.m_SvSpectatorSlots;
}
